...
- **Verified external implementation fact — 2026-08-06 through 2026-08-07:** `PaintSwitch AI Assistant` exists in GoHighLevel as a Prompt Based bot and has only Live Chat selected. It is the only bot and was automatically marked Primary. The D-052-approved OpenAI GPT-4.1 model and safety-first prompt are saved, four bounded provider-panel tests passed, and the D-053 Live Chat widget/settings were saved. The bot was changed from Off to Auto Pilot on 2026-08-07 solely for controlled owner testing. Saved settings include a 10-reply cap, two-second delay, images/voice responses off, and manual/workflow sleep; Conversation Summary and transcript export remained off in the prior inspection. No chatbot action is configured.
