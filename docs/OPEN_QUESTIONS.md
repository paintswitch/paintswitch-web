...
- **Verified implementation fact — 2026-08-03:** The private integration, least-required beta permissions, Sensitive Vercel GHL configuration, location/pipeline/stage identifiers, seven custom-field identifiers, four additional Opportunity fields, native/source mappings, free Upstash database `paintswitch-lead-idempotency` in `iad1`, and five Sensitive Upstash Production-and-Preview variables are configured. Pushed commit `efb96b9` includes the REST clients, durable phases, reconciliation, and failure recovery and is available in a fail-closed Vercel Preview. Retention/TTL approval, enabled production delivery, owner notification, and real-provider end-to-end evidence remain pending.
...
- **Verified implementation fact — 2026-08-03:** The GHL and Upstash configuration, pushed implementation, Ready Preview, and forty-one local tests exist. Retention/TTL approval, enabled production delivery, owner notification, production promotion, and real-provider acceptance evidence remain incomplete.
...
- **TBD:** The owner notification destination remains unresolved. D-022 approved and the GoDaddy attempt used `hello@paintswitch.com`, but the owner's latest text used `hell@paintswitch.com`. Treat the latter as a possible typo, not a superseding decision, until the owner explicitly clarifies the destination. Any staff ownership beyond the confirmed PaintSwitch owner role also requires clarification.
