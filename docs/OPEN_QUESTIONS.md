...
- **Confirmed:** Send owner lead notifications to `hello@paintswitch.com`. The owner clarified the exact address on 2026-08-03, resolving the temporary `hell@paintswitch.com` typo conflict. Owner-supplied photographic evidence confirms receipt of the separately sent test email. A workflow is saved in Draft and remains unpublished and untested as described below.
- **Confirmed:** Use From name `PaintSwitch` and From email `hello@paintswitch.com` for the internal owner lead-notification email. Approved on 2026-08-03 under D-031. This resolves the internal From-name and From-email choice only; authentication, deliverability, the actual inbox sender line, customer-facing sender scope, workflow publication, failure handling, and end-to-end operation remain unresolved or unverified.
...
- **Verified implementation fact — 2026-08-03:** The private integration, least-required beta permissions, Sensitive Vercel GHL configuration, location/pipeline/stage identifiers, seven custom-field identifiers, four additional Opportunity fields, native/source mappings, free Upstash database `paintswitch-lead-idempotency` in `iad1`, five Sensitive Upstash Production-and-Preview variables, and saved Draft workflow `PaintSwitch Website Lead - Owner Notification` are configured. The Draft triggers on Opportunity Created for `PaintSwitch Lead Intake`, contains an internal Email action to `hello@paintswitch.com`, and now has D-031 From name `PaintSwitch` and From email `hello@paintswitch.com` saved; it remains unpublished and untested as a workflow. Owner-supplied photographic evidence confirms receipt of an earlier separate test whose sender was `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. After the D-031 values were saved, a controlled second test send completed in the GoHighLevel UI without a visible error, but receipt and the actual inbox sender line remain unverified. Pushed commit `efb96b9` includes the earlier REST clients, durable phases, reconciliation, and failure recovery and is available in a fail-closed Vercel Preview. Current source also implements D-030's 30-day technical-record TTL and client-IP rate limit, with fifty-one passing automated tests plus passing lint, TypeScript type checking, and production build. The safeguard changes have no hosted verification; enabled production delivery, operational owner notification, and real-provider end-to-end evidence remain pending.
...
- **Verified implementation fact — 2026-08-03:** The GHL and Upstash configuration, earlier pushed implementation and Ready Preview, fifty-one locally passing automated tests, locally passing lint, TypeScript type checking, and production build, and saved Draft owner-notification workflow exist. Receipt of the earlier separate test email at `hello@paintswitch.com` is confirmed. The Draft now contains the D-031 From values, and a controlled second test send completed in the GoHighLevel UI without a visible error, but receipt and the actual inbox sender line remain unverified. The workflow remains unpublished and untested as a workflow. The approved 30-day technical-record TTL and client-IP rate limit are implemented and locally verified, but have no hosted verification. Enabled production delivery, operational owner notification, production promotion, and real-provider acceptance evidence remain incomplete.
- **Confirmed:** Remove placeholder reviews for the beta and do not restore reviews until verified, permissioned reviews exist.
- **Verified implementation fact — 2026-08-03:** Native name/phone/email and approved Opportunity-field mappings, private-integration/token creation, Vercel GHL configuration, provisioned Upstash database/variables, durable state/locking, provider-failure reconciliation, explicit rejected-create recovery, sanitized submission-ID error reporting, locally verified D-030 safeguards, the saved D-031 sender values, and fifty-one local tests exist. Vercel automatically deployed the earlier Ready Preview, and a hosted valid submission failed closed while delivery was disabled. Pending work includes corrected-sender receipt verification, hosted safeguard verification, enabled delivery, notification, production promotion, production-safe provider testing, and final operating acceptance within D-024 through D-031.
...
- **Confirmed:** The owner clarified on 2026-08-03 that the notification destination is `hello@paintswitch.com`; `hell@paintswitch.com` was a typo and must not be used. D-031 separately confirms the internal sender as `PaintSwitch <hello@paintswitch.com>`. The Draft contains those From values, but corrected-sender receipt, authentication/deliverability, workflow testing/publication, and failure handling remain open.
...
- **TBD:** Complete outbound, credentials/recovery, sender authentication/deliverability, actual inbox sender-line verification, and GoHighLevel workflow acceptance. The earlier separate test was received with sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. The D-031 replacement values are saved and a second UI test send completed without a visible error, but the owner has not verified receipt or the delivered sender line. Neither test nor the unpublished Draft is end-to-end notification evidence.
