...
- **Verified repository and CI implementation fact — 2026-08-07:** Commit `649f06d` contains the homepage-mounted, `HIGHLEVEL_CHAT_WIDGET_ENABLED`-gated `next/script` component with `lazyOnload`, public widget ID `6a75455fa70a87ea8ede056f`, exact LeadConnector CSP origins, source-contract tests, and expanded Privacy content. The commit was pushed on `codex/lead-generation-beta`, and PR #2 passed `GitHub Verify`, `Vercel`, and `Vercel Preview Comments`.
- **Verified hosted Preview and manual synthetic deletion fact — 2026-08-07:** Preview `https://paintswitch-9vzqs7zmi-paint-switch.vercel.app` loaded the widget only on the homepage. Its visible title and intro disclosed AI. It showed no pre-chat contact form or visible attachment/voice control, but provider markup retained a hidden `display:none` file input; provider-internal attachment code is not claimed absent. A service question produced the four approved service categories and `Request a Quote` direction. A pricing/booking request was refused and directed to the form/human follow-up. Those two replies appeared after page reload. For a third question asking whether ZIP `22102` could be confirmed, HighLevel generated a refusal to guarantee ZIP availability and routed to `Request a Quote`/human review, but the browser widget did not display it during a 30-second no-reload observation. Real-time delivery/latency remains unaccepted. Browser logs contained only unrelated Chrome-extension warnings and no site/widget error. The exact synthetic conversation `Guest Visitor Svdtu`, containing no email, phone, or customer data, was permanently deleted through `Delete Conversation` > `Delete Forever`; the inbox then showed no unread conversations. This passes one manual synthetic deletion rehearsal, not provider-enforced retention, anonymization, backup deletion, or ongoing operation. Mobile, both shutdown paths, Production runtime, provider retention/anonymization, and promoted release remain unverified.
