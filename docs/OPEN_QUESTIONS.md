...
- **Confirmed:** Publish the exact `PaintSwitch Website Lead - Owner Notification` workflow under D-032 dated 2026-08-04. Publication persisted after reload. One controlled manual Opportunity and one later protected Preview website submission each produced exactly one completed enrollment and a successful internal-notification action; the owner confirmed receipt of the Preview-triggered message at approximately 1:20 PM EDT. Failure handling, conditional-backup operation, branded-domain authentication, and production end-to-end operation remain unverified.
...
- **Verified external fact — 2026-08-04:** The corrected controlled email reached `hello@paintswitch.com` with visible sender `PaintSwitch`, routing address `hello+paintswitch.com@mail.msgsndrroute.com`, the approved subject/body, and no Jen identity, satisfying AC-044. The exact workflow remained Saved/published after reload. A manual CRM trigger and a later protected Preview website trigger each completed once with a successful internal-notification action; the owner confirmed receipt of the Preview-triggered message at approximately 1:20 PM EDT.
- **TBD:** Production notification acceptance, failure handling, branded-domain authentication, payment-processor configuration, post-beta modules, production ownership not already assigned, customer-facing sender identity, and release phasing.
...
- **Verified implementation fact — 2026-08-04:** Safeguarded commit `3bac3ba` passed one controlled, access-protected Preview website path while Preview delivery alone was temporarily enabled and Production remained disabled. Submission `a0c9e9cd-4e98-4f45-8c1f-16c9a31256ec` produced matching Contact `w1tEl1ktbKestabbkBr3`, Opportunity `11u1dIja7oMJXS4srkPu`, exact field and source/campaign mappings, one successful owner-notification workflow action, and the approved on-page confirmation. The owner confirmed receipt of the resulting internal message at approximately 1:20 PM EDT. The Preview environment setting and latest branch Preview were restored to `false` on a fresh Ready deployment; the immutable enabled-test Preview remains access-protected. Failure/retry/concurrency behavior, production promotion, and production end-to-end acceptance remain incomplete.
- **Confirmed:** Remove placeholder reviews for the beta and do not restore reviews until verified, permissioned reviews exist.
- **Verified implementation fact — 2026-08-04:** The D-031 sender evidence and D-032 published state are verified. In addition to the earlier manual trigger, the protected Preview website test produced exactly one completed enrollment and successful internal-notification action with no SMS or other customer action, and the owner confirmed receipt of that message. Upstash recorded a `completed` state with matching provider IDs, payload hash, completion phase, version, and updated timestamp but no raw submitted intake fields, a near-30-day TTL, no residual lock, and one Preview rate-counter increment. Customer-facing Privacy and Terms pages and the approved pre-submit disclosure are implemented locally. Pending work includes the hosted five-submission threshold, failure/retry/concurrency paths, production promotion, privacy/retention operating procedures, professional/legal review, human-response rehearsal, conditional-backup setup, and final operating acceptance within D-024 through D-046.
...
- **Confirmed:** The destination is `hello@paintswitch.com`, the internal sender is `PaintSwitch <hello@paintswitch.com>`, and the exact workflow is published. Corrected-sender receipt/no-Jen evidence and owner-visible receipt of the protected-Preview website notification are verified. Failure handling, branded-domain authentication, conditional-backup operation, and Production acceptance remain open.
- **Confirmed:** Alex is the conditional backup only after `alex@paintswitch.com` is created and a controlled receipt test passes. The owner reported mailbox creation on 2026-08-04. Receipt/access, exact routing, and a response rehearsal remain incomplete.
...
- **Confirmed:** Apply a 30-day TTL to Upstash technical idempotency/delivery-state records. Current source and local tests apply the TTL to every guarded technical-state write. The protected Preview test observed 2,591,535 seconds remaining on its completed state containing no raw submitted intake fields and no residual lock, providing one hosted success-path observation. This remains separate from the confirmed 12-month rule for unconverted CRM leads, and failure/retry/concurrency behavior remains unverified.
- **Confirmed:** Enforce no more than five beta lead submissions per client IP address per ten minutes. Current source uses an atomic fixed window that begins with the first valid request, counts only valid submissions, and does not increment or extend the window for blocked attempts. Client-IP buckets use HMAC pseudonyms and separate production, preview, and local namespaces; token rotation resets active pseudonymous buckets. Deployed source trusts only `x-vercel-forwarded-for`, localhost uses a shared bucket, and unavailable limiting fails closed. Local tests pass. The protected Preview test observed one Preview rate key with count `1` and 101 seconds remaining; the five-submission threshold and `429` behavior were not exercised in the hosted environment.
...
- **TBD:** Complete branded-domain authentication, credentials/recovery, failure handling, and production end-to-end GoHighLevel workflow acceptance. The corrected controlled message satisfies no-Jen visible-sender evidence; the manual CRM trigger and protected Preview website trigger each verified one successful internal-notification action, and the owner confirmed receipt of the Preview-triggered message. This does not prove failure recovery or production operation.
