...
- Treat D-046 and D-048 as the conditional escalation-only backup rule: the immediate new-lead notification remains at `hello@paintswitch.com`, and Alex must not receive every lead. `alex@paintswitch.com` may receive an internal backup alert only after a separately approved escalation signal indicates that backup attention is needed.
- Use `BETA_OPERATIONS_RUNBOOK.md` for beta execution and evidence capture. Alex mailbox existence, direct access, and basic self-send/receipt are verified. External-domain receipt, GoHighLevel escalation routing, activation, and human-response rehearsal are not verified. The published workflow currently triggers on Opportunity Created in `PaintSwitch Lead Intake` and contains only the immediate `Email Owner - hello@paintswitch.com` action; the pipeline still has exactly one `New Quote Request` stage. Although GoHighLevel exposes Wait and If/Else actions, no reliable handled/escalation signal currently exists.
- Treat a second internal pipeline stage named `Contact Attempted` only as **Proposed**. It could provide a reliable handled signal for a later Wait/If/Else escalation branch, but adding it would change the confirmed one-stage pipeline and requires explicit owner approval before implementation. Do not infer an escalation delay, transition rule, notification message, or workflow behavior that has not been approved.
