...
PR #2 was squash-merged to `main` as commit `7fb7359ccd5d1518d2dd4763e34350da9168d0cb`. Vercel Production deployment `paintswitch-ghh6fv5dl-paint-switch.vercel.app` reached `Ready`, and `https://paintswitch.com` loaded the merged homepage. The page displayed the chat prompt and opened a widget titled `PaintSwitch AI Assistant` with an AI-disclosing intro. Provider markup contained a hidden file input, but it was not visible. The live chat launcher count was zero on `/privacy` and `/terms` and one after returning to `/`, verifying the homepage-only mount. Those route checks produced zero non-extension site/widget console logs. No Production chat message was sent, so this verifies widget presence and opening only, not a Production AI reply or latency.
...
- The chat widget ID is public configuration in source. No HighLevel chatbot credential or model secret is present. The enabled environment flag and the provider bot's Auto Pilot state were exercised together in Preview and the widget opened in Production, proving the activation side of the two-control boundary, one Preview no-reload reply display, and Production presence/opening only. Neither shutdown path has been tested, and Production replies plus exact delay/cap/sleep runtime behavior remain unverified. Production showed no visible attachment control, but provider markup retained a hidden file input; provider-internal attachment code is therefore not claimed absent.
