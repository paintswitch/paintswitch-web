...
Last repository inspection: 2026-08-07

Last external runtime verification: 2026-08-07
...
- `next.config.ts` disables `X-Powered-By` and applies a static-compatible CSP plus frame, MIME-sniffing, referrer, permissions, and cross-domain-policy response headers. Commit `649f06d` evaluates `HIGHLEVEL_CHAT_WIDGET_ENABLED === "true"`. When false, the prior self-only browser-resource policy remains. When true, `script-src`, `img-src`, and `connect-src` add only `https://widgets.leadconnectorhq.com`, `https://services.leadconnectorhq.com`, and `https://stcdn.leadconnectorhq.com`; no wildcard is used, and `frame-src` remains `'none'`. Production retains the minimal `'unsafe-inline'` script/style allowances required by Next.js hydration and built-in error-page output; development alone adds `'unsafe-eval'` and WebSocket connections. The exact enabled policy loaded the widget in the hosted Preview without a site/widget browser error; Production runtime remains unverified.
...
Application commit `649f06d`, also available in the hosted Vercel Preview verified below, contains:
...
- The repository contains one GitHub Actions verification workflow. PR #2 passed GitHub `Verify`, `Vercel`, and `Vercel Preview Comments` for application commit `649f06d` on 2026-08-07.
- The repository contains no tracked Vercel project configuration; `.vercel` is ignored.
- `next.config.ts` disables `X-Powered-By` and configures the security-header baseline. Commit `649f06d` conditionally adds the three exact LeadConnector origins only when the chat environment flag is true. The enabled policy loaded the hosted Preview widget without a site/widget browser error; Production runtime remains unverified.
- Pushing `codex/lead-generation-beta` automatically creates Vercel Previews. Application commit `649f06d` produced Preview `https://paintswitch-9vzqs7zmi-paint-switch.vercel.app`, where the bounded desktop chatbot test passed within the limits above. The protected lead-delivery operational tests recorded above were performed on earlier application commit `beb6781`; both its tested enabled state and restored disabled redeployment reached `Ready`. Production promotion/release and rollback remain unverified, and the apex still serves the prior `main` build.
