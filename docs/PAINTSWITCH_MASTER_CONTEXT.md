...
- **Verified external evidence / corrected configuration — 2026-08-03:** An earlier received separate test displayed sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>` together with the approved internal-alert body. The Draft now has the D-031 From name `PaintSwitch` and From email `hello@paintswitch.com` saved. A controlled second test send completed in the GoHighLevel UI without a visible error, but the owner has not verified receipt or the actual inbox sender line. Saved configuration and UI completion are implementation evidence, not deliverability or workflow-operation proof.
...
- **Verified implementation fact — 2026-08-03:** The private integration, least-required permissions, Sensitive Vercel GHL configuration, identifiers, delivery implementation, free Upstash database, five Sensitive Upstash Production-and-Preview variables, D-030 technical safeguards, and a saved Draft owner-notification workflow exist. Current source applies a 30-day TTL to every guarded Upstash technical delivery-state write and atomically limits each HMAC-pseudonymized client-IP bucket to five valid submissions in a fixed 600-second window. Fifty-one automated tests, lint, TypeScript type checking, and the production build pass locally. Production-hosted safeguard verification, enabled delivery, operational notification, and provider end-to-end verification remain incomplete. The Draft uses an Opportunity Created trigger filtered to `PaintSwitch Lead Intake`, an internal Email action for `hello@paintswitch.com`, From name `PaintSwitch`, and From email `hello@paintswitch.com`; it is unpublished and untested as a workflow. Owner-supplied photographic evidence confirms that an earlier separately sent test email reached the mailbox but displayed a Jen-linked sender. A controlled second test send completed in the GoHighLevel UI without a visible error after the D-031 values were saved, but receipt and the actual inbox sender line remain unverified.
...
- **Confirmed:** `hello@paintswitch.com` is also the owner-notification destination. Receipt of an earlier separate test at that mailbox is confirmed by owner-supplied photographic evidence. D-031 replaces its observed Jen-linked sender with the approved internal From values, which are saved in the Draft; corrected-sender receipt and the actual inbox sender line remain unverified, and the workflow remains Draft and unpublished. Approved on 2026-08-03 under D-029 and D-031.
- **Confirmed:** The internal owner-notification sender is `PaintSwitch <hello@paintswitch.com>`. Those exact values are saved in the Draft under D-031. A controlled second test send completed in the GoHighLevel UI without a visible error, but receipt and the actual inbox sender line remain unverified; the workflow remains Draft and unpublished.
...
- **Verified implementation fact — 2026-08-03:** The approved private integration, least-required beta permissions, server-only Vercel GHL configuration, location/pipeline/stage configuration, four additional Opportunity fields, native/contact and Opportunity mappings, durable delivery phases, token-checked 60-second lock, 30-day technical-state TTL, HMAC-pseudonymized fixed-window client-IP rate limiter, streamed response caps, reconciliation, explicit rejected-create recovery, free Upstash database, Sensitive Upstash variables, automated tests, and the saved Draft `PaintSwitch Website Lead - Owner Notification` workflow exist. These are implementation details within D-024 through D-031, not additional owner decisions. Fifty-one automated tests, lint, TypeScript type checking, and the production build pass locally. The earlier candidate is available in a Ready Vercel Preview, but the safeguard changes have not been hosted-verified and `LEAD_DELIVERY_ENABLED` is `false`. An earlier received separate test displayed a Jen-linked sender. The Draft now has the D-031 sender values saved, and a controlled second test send completed in the GoHighLevel UI without a visible error, but receipt and the actual inbox sender line remain unverified. The Draft remains unpublished. Real-provider delivery, operational notification, production promotion, and production end-to-end evidence remain incomplete.
