...
- **Verified external evidence / implementation conflict — 2026-08-03:** The received separate test displayed sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>` together with the approved internal-alert body. The Jen-linked sender identity conflicts with the standalone-brand decision and must be replaced and verified as PaintSwitch-only before the workflow is published or used for customer-facing communication. The observed sender is implementation evidence, not owner approval.
- **TBD:** Production staff ownership beyond the confirmed owner role, backup lead ownership, customer-facing sender authentication, payment-processor configuration, remaining enabled modules, and release phasing. Account credentials and configured identifiers are implementation facts, not additional owner-approved product policy.
...
- **Confirmed:** `hello@paintswitch.com` is also the owner-notification destination. Receipt of the separate test email at that mailbox is confirmed by owner-supplied photographic evidence, but the workflow remains Draft and unpublished and the observed Jen-linked sender identity must be corrected before publication. Approved on 2026-08-03 under D-029.
