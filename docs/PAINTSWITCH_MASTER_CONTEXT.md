...
- **Confirmed:** The PaintSwitch owner is the primary operational recipient. Alex is only a conditional backup after `alex@paintswitch.com` is created and a controlled receipt test passes. The owner reported mailbox creation on 2026-08-04; receipt, access, routing, and backup-response operation remain unverified.
...
- **Verified implementation fact — 2026-08-04:** The approved integration, mappings, durable safeguards, published owner-notification workflow, corrected received-message evidence, one successful controlled manual Opportunity-trigger execution, and one protected Preview-only website-to-inbox success-path execution exist as described under D-024 through D-032. The current local candidate adds working `/privacy` and `/terms` pages, approved form disclosures, Virginia-prioritized individual service-area review copy, and the beta operations runbook. Under D-047, exact stable Next.js and `eslint-config-next` `16.3.0` are installed; the production audit reports zero vulnerabilities; and fifty-seven automated tests, lint, TypeScript type checking, and a production build reporting `/`, `/privacy`, `/terms`, `/_not-found`, and `/api/leads` pass locally. Local production smoke verifies all authored pages, security headers, no `X-Powered-By`, recoverable fail-closed form behavior with value retention, and no prohibited Jen/private-address/MHIC/DMV built content. Preview evidence verifies one completed technical state with the expected TTL, one rate-counter increment, exact CRM mappings, source/campaign attribution, on-page confirmation, a successful workflow action, and owner-confirmed inbox receipt. The hosted rate-limit threshold, retries/concurrency, failure recovery, human-response timing, privacy/retention rehearsal, professional/legal review, production promotion, and production end-to-end evidence remain incomplete. The Preview environment setting and latest branch Preview were restored to fail closed after testing, Production remains `false`, and the prior immutable enabled Preview remains access-protected.
...
- **TBD:** Professionally/legally verify the locally implemented Privacy, Terms, contact, age, and SMS-off disclosures; rehearse the runbook's confirmed 12-month deletion/anonymization, privacy-request verification, exception-audit, and response-hour procedures; verify branded-domain authentication, hosted failure-path recovery, retry/concurrency behavior, five-submission threshold enforcement, and the Production website-to-CRM-to-notification path; receipt-test `alex@paintswitch.com` before backup routing; verify exact DBA filing/registrant/address/tax/authorized-representative/licensing and any later A2P facts; define retention for record classes outside unconverted CRM leads; decide any abuse controls beyond D-030's confirmed beta rate limit; define detailed events; and assign later-release sequencing. Provider or carrier approval is not legal approval.
