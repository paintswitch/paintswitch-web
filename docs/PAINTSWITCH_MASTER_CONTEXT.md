...
- **Confirmed:** Send owner lead notifications to `hello@paintswitch.com`. The owner resolved the temporary `hell@paintswitch.com` typo conflict on 2026-08-03. This confirms the destination, not mailbox provisioning or workflow operation.
...
- **Verified implementation fact — 2026-08-03:** The private integration, least-required permissions, Sensitive Vercel GHL configuration, identifiers, delivery implementation, free Upstash database, five Sensitive Upstash Production-and-Preview variables, and a saved draft owner-notification workflow exist. The draft uses an Opportunity Created trigger filtered to `PaintSwitch Lead Intake` and an internal Email action for `hello@paintswitch.com`. It is unpublished and untested. Retention/TTL approval, enabled delivery, operational notification, and provider end-to-end verification do not exist.
...
- **Confirmed:** `hello@paintswitch.com` is also the owner-notification destination. The mailbox and notification workflow remain non-operational until provisioning and end-to-end testing pass. Approved on 2026-08-03 under D-029.
...
- **Verified implementation fact — 2026-08-03:** The approved private integration, least-required beta permissions, server-only Vercel GHL configuration, location/pipeline/stage configuration, four additional Opportunity fields, native/contact and Opportunity mappings, durable delivery phases, token-checked 60-second lock, streamed response caps, reconciliation, explicit rejected-create recovery, free Upstash database, Sensitive Upstash variables, automated tests, and the saved draft `PaintSwitch Website Lead - Owner Notification` workflow exist. These are implementation details within D-024 through D-029, not new owner decisions. The candidate is available in a Ready Vercel Preview, but `LEAD_DELIVERY_ENABLED` is `false`; the notification workflow is unpublished and untested; retention/TTL approval, real-provider delivery, operational notification, production promotion, and production end-to-end evidence remain incomplete.
