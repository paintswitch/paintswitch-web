...
- **Consequences:** The beta must not display instant prices, collect deposits or payments, enable checkout, promise customer-selected scheduling, or imply that an address or price is approved before review. It may state that PaintSwitch serves the DMV while disclosing that service availability and pricing are confirmed after review. The confirmed customer-facing chatbot, instant quote, checkout, payments, photo-review automation, and automated proposals are deferred from this beta rather than rejected. Decision D-019 later confirms the primary lead owner, acknowledgment channels, and Vercel beta hosting. D-030 later confirms the beta's rate-limit threshold and technical Upstash-record TTL; current source implements them and local tests pass, while hosted verification remains pending. Exact GoHighLevel configuration, pipeline fields beyond the approved intake data, backup owner, measurable human-response target and operating hours, SMS content/consent/sender/workflow, legal wording, broader CRM/business-data retention and deletion, detailed conversion-event definitions, and later-release sequencing remain TBD until approved or verified.
...
- **Superseded by:** D-027 supersedes only the statement that SMS must be enabled to satisfy the beta. The pipeline, stage, original fields, sample-data, and interim-disabled-state portions remain current.
...
- **Consequences:** The three custom fields may be created in GoHighLevel and used by the website mapping. **Implementation verification — 2026-08-03:** The fields and local mappings exist, and the server-side field identifiers are configured. D-029 confirms `hello@paintswitch.com` as the owner-notification destination, and D-031 confirms the internal notification sender as `PaintSwitch <hello@paintswitch.com>`. A saved Draft notification workflow contains those From settings, but staff ownership in GoHighLevel, workflow publication/testing, real Upstash runtime verification, production deployment, sender authentication/deliverability, and provider end-to-end tests remain incomplete. Owner-supplied photographic evidence confirms that an earlier separately sent test email with subject `New PaintSwitch website lead` reached the mailbox, but it displayed the Jen-linked sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. A later controlled test send completed in the GoHighLevel UI without a visible error after the D-031 settings were saved; its receipt and actual inbox sender line remain unverified. Neither test proves workflow or end-to-end acceptance.
...
- **Consequences:** The date is a delivery goal, not authorization to bypass any confirmed safety, legal, privacy, consent, security, messaging, or acceptance requirement. GoHighLevel credentials and secrets must not be exposed to customer-delivered code. **Implementation verification — 2026-08-03:** The custom form, private integration, server-only GHL configuration, mappings, at-most-one automatic-create/reconciliation logic, failure handling, Upstash database, Sensitive Vercel connection, D-030 technical-record TTL, and D-030 client-IP rate limit exist in current source. Fifty-one automated tests, lint, TypeScript type checking, and the production build pass locally. The earlier candidate remains available in a fail-closed Vercel Preview, but the safeguard changes have no hosted verification and `LEAD_DELIVERY_ENABLED` remains `false`. Notification, enabled real-provider testing, production promotion, legal/privacy/consent, broader CRM/business-data retention and deletion, and final operating evidence remain incomplete. Live production still serves the prior site with Drywall Repair, placeholder testimonials, nonfunctional Privacy/Terms links, nonworking estimate/contact CTAs, and universal-consultation copy. Vercel shows Hobby, so the owner must approve a commercially compliant plan before commercial launch; no plan purchase is approved. External A2P remains a schedule risk, but D-027 permits launch with SMS disabled while pending. Work must be sequenced so required owner participation does not exceed one hour on any day.
...
- **Consequences:** Private-integration creation and Vercel secret configuration may proceed. Use only the permissions required for the approved beta contact/opportunity delivery and verify the environment-variable handling, rotation procedure, native mappings, delivery workflow, and end-to-end behavior during implementation. The confidential token value must never enter canonical documentation. D-029 confirms the owner-notification destination, and D-031 confirms the internal notification From name and email. The Draft has those fields saved, and a controlled second test send completed in the GoHighLevel UI without a visible error; receipt, the actual inbox sender line, sender authentication/deliverability, workflow publication, failure handling, and workflow end-to-end testing remain incomplete. **Implementation verification — 2026-08-03:** The private integration now exists with the required beta contact/opportunity/custom-field permissions, and its token is stored as a Sensitive Vercel environment variable for Production and Preview. Location, pipeline, and stage identifiers are also configured server-side. This proves configuration, not deployment or successful provider delivery; `LEAD_DELIVERY_ENABLED` remains `false`.
...
- **Consequences:** The Upstash account/database and server-only Vercel secrets may be configured and the lead-delivery path may use the approved submission ID for durable duplicate protection. D-030 fixes the durable technical idempotency/delivery-state TTL at 30 days. Key design, atomic operation, provider-failure behavior, reconciliation, limits, monitoring, and acceptance tests remain implementation details to complete and verify within this confirmed boundary. No Upstash credential may enter customer-delivered code or tracked source. **Implementation verification — 2026-08-03:** The owner accepted the marketplace terms. Free database `paintswitch-lead-idempotency` exists in Washington, D.C. (`iad1`) with eviction disabled and is connected to Vercel through five Sensitive Production-and-Preview variables. Current source contains a shared validated Upstash configuration loader; a token-checked 60-second lock; a 30-day expiry on every guarded technical-state write; durable delivery phases; streamed response-size caps; provider-failure reconciliation; explicit rejected-create recovery; and local fault-injection tests for at-most-one automatic Opportunity creation, recovery from state-write failures, and the required TTL. Delivery remains disabled, hosted Upstash behavior remains unverified, and no real Upstash or GoHighLevel end-to-end delivery has passed. D-030 does not approve retention for CRM/business/customer data.
...
- **Consequences:** Configure the owner-notification workflow to use `hello@paintswitch.com`; never use `hell@paintswitch.com`. This decision confirms the destination only; D-031 confirms the internal-notification sender. **Implementation verification — 2026-08-03:** Saved Draft workflow `PaintSwitch Website Lead - Owner Notification` uses an Opportunity Created trigger filtered to `PaintSwitch Lead Intake` and an internal Email action directed to `hello@paintswitch.com`; it remains unpublished. Owner-supplied photographic evidence confirms that an earlier separately sent test email with subject `New PaintSwitch website lead` reached the PaintSwitch mailbox and displayed the approved internal-alert body, but its sender was `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. The Draft now has From name `PaintSwitch` and From email `hello@paintswitch.com` saved under D-031. A controlled second test send completed in the GoHighLevel UI without a visible error, but the owner has not verified receipt or the actual inbox sender line. Lead delivery and notification must not be represented as operational until sender authentication/deliverability, workflow publication, failure handling, and end-to-end acceptance evidence are complete.
...
- **Consequences:** The internal owner-notification workflow must use exactly those From values and must not use a Jen Contracting-linked sender. This decision confirms only the internal notification sender choice. Sender authentication, deliverability, the actual inbox sender line, customer-facing sender scope, workflow publication, failure handling, and end-to-end operation remain **TBD** or unverified as labeled. **Implementation verification — 2026-08-03:** The saved Draft workflow now contains From name `PaintSwitch` and From email `hello@paintswitch.com`. A controlled second test send completed in the GoHighLevel UI without a visible error, but the owner has not verified receipt or the delivered inbox sender line. The Draft remains unpublished, and no website-to-CRM-to-notification test has passed.
