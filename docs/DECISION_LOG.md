...
## D-001 — Standalone PaintSwitch brand
...
- **Date:** 2026-07-17
- **Status:** Confirmed
- **Decision:** Position PaintSwitch as mid-market, convenience-first, technology-first, and technology-driven mainstream, not as an extension of Jen Contracting's luxury positioning.
- **Reason:** Aligns the brand with a clear, accessible, technology-enabled buying experience.
- **Consequences:** Product and messaging should emphasize convenience, clarity, speed, and mainstream accessibility without inheriting Jen Contracting positioning.
- **Supersedes:** Any prior working assumption that PaintSwitch should inherit Jen Contracting's luxury positioning. No prior decision ID is documented.

## D-004 — Paint supplied by PaintSwitch

- **Date:** 2026-07-17
- **Status:** Confirmed
- **Decision:** PaintSwitch supplies the paint.
- **Reason:** Establishes responsibility for a core project input.
- **Consequences:** Pricing, operations, and customer expectations must treat paint as PaintSwitch-supplied. Exact brands, grades, finishes, and allowances remain TBD.
- **Supersedes:** None documented.

## D-005 — Hybrid labor model

- **Date:** 2026-07-17
- **Status:** Confirmed
- **Decision:** Labor uses a hybrid model of employee and subcontractor crews.
- **Reason:** Establishes the approved high-level workforce model.
- **Consequences:** Future operating, scheduling, quality, compliance, and customer-experience requirements must support both crew types without making unapproved promises about assignment.
- **Supersedes:** None documented.

## D-006 — Four marketed services with controlled sales lanes

- **Date:** 2026-07-29 for marketed services and sales-lane limits. Residential interior as the operational V1 is Confirmed; its exact earlier historical source date remains uncertain and the rule was reaffirmed on 2026-08-01.
- **Status:** Confirmed
...
- **Date:** 2026-07-29
- **Status:** Confirmed
- **Decision:** Calculate qualifying interior pricing room by room.
- **Reason:** Makes each room's scope explicit and creates a consistent calculation unit.
- **Consequences:** Quote intake and future pricing logic must preserve room-level inputs and calculate the project from those room-level results.
- **Supersedes:** Any undocumented whole-project-only pricing assumption. No prior decision ID is documented.

## D-008 — Room-size tiers

- **Date:** 2026-07-30
- **Status:** Confirmed
- **Decision:** Room square footage equals length multiplied by width. Small is 120 sq ft or less; Medium is 121–200 sq ft; Large is 201–300 sq ft; Oversized is more than 300 sq ft and requires dimensions and photo review.
- **Reason:** Defines approved, testable size classifications for room-by-room quoting.
- **Consequences:** Boundary handling must be exact. Oversized rooms cannot remain in unsupported automated firm pricing.
- **Supersedes:** Any prior undocumented room-size bands. No prior decision ID is documented.

## D-009 — Required quote inputs

- **Date:** 2026-07-31 (documented as confirmed; earlier approval date not provided)
- **Status:** Confirmed
- **Decision:** Quote inputs include room type and size, walls, ceilings, baseboards/trim/crown, doors, closets, ceiling height, repairs, color changes, and occupied/furniture level.
- **Reason:** Establishes the minimum categories of project information needed for the approved room-based model.
- **Consequences:** Future quoting must represent these inputs at room level where applicable. Exact choices, thresholds, and pricing effects remain TBD unless separately confirmed.
- **Supersedes:** None documented.

## D-010 — Ceiling pricing remains unapproved

- **Date:** 2026-07-31 (documented as confirmed; earlier approval date not provided)
- **Status:** Confirmed
- **Decision:** Exact ceiling-height tiers and surcharges are not approved.
- **Reason:** Prevents working assumptions from becoming unsupported live pricing.
- **Consequences:** Ceiling height must be collected, but no firm-price rule based on an unapproved tier, multiplier, or surcharge may be used.
- **Supersedes:** Any unapproved ceiling-tier or surcharge assumption. No prior decision ID is documented.

## D-011 — No-call qualifying sale and online booking

- **Date:** 2026-07-11 for quick quotes and closing qualifying sales before a required call; 2026-07-17 for instant pricing and online booking
- **Status:** Confirmed
- **Decision:** The launch model includes instant pricing and online booking, with the goal of providing quick quotes and closing qualifying sales online before anyone needs to call the customer.
- **Reason:** Implements PaintSwitch's convenience-first, technology-first positioning.
- **Consequences:** The qualifying standard-interior journey must be capable of reaching firm price, checkout, and booking without a mandatory pre-sale call. Exact checkout, deposit, and scheduling rules remain TBD or Proposed.
- **Supersedes:** Any working assumption that a phone call is mandatory before every qualifying sale. No prior decision ID is documented.

## D-012 — Customer-facing chatbot
...
- **Date:** 2026-07-16
- **Status:** Confirmed
- **Decision:** Apply one flat $125 fuel/materials adjustment to every project.
- **Reason:** Establishes the approved project-level fuel/materials adjustment.
- **Consequences:** Every project price must include the adjustment exactly once. It is separate from unapproved base prices, travel-zone adjustments, ceiling surcharges, repair allowances, coat rules, minimums, deposits, and cancellation policies.
- **Supersedes:** The incomplete 2026-07-31 documentation statement that no numeric pricing value was confirmed. No prior decision ID documented the $125 adjustment.

## D-014 — GoHighLevel launch backbone
...
- **Date:** 2026-07-18
- **Status:** Confirmed
- **Decision:** Use the first HTML example's logo direction as the owner-approved visual preference: a paint roller with paint behind it.
- **Reason:** Records the owner's preferred visual direction for PaintSwitch identity work.
- **Consequences:** Future logo exploration should follow this direction unless superseded. No specific production logo file, final artwork, variant set, or usage specification is confirmed by this preference alone.
- **Supersedes:** None documented.

## D-017 — Ten-minute launch stand-ups

- **Date:** 2026-07-18
- **Status:** Confirmed
- **Decision:** Launch stand-ups should be 10 minutes, not 30 minutes.
- **Reason:** Records the owner's preferred operating cadence for concise launch coordination.
...
- **Date:** 2026-08-06
- **Status:** Confirmed
- **Decision:** Use OpenAI GPT-4.1 for the GoHighLevel Prompt Based bot then named `PaintSwitch AI Assistant`. Its V1 is prompt-only: the first reply must disclose that the visitor is speaking with an AI assistant; the bot may answer only from **Confirmed** PaintSwitch facts and direct visitors to `Request a Quote` or human review. It must not provide pricing estimates, confirm project or service availability, book or schedule work, take or arrange payments, claim that a lead or form was submitted, collect sensitive data, or discuss any relationship between PaintSwitch and another company. D-054 later changes only the name to `Switch bot`.
...
### Production deployment and widget-opening evidence — 2026-08-08

This is implementation evidence only and creates no new decision. Pull request #2 was squash-merged to `main` as commit `7fb7359ccd5d1518d2dd4763e34350da9168d0cb`. Vercel Production deployment `paintswitch-ghh6fv5dl-paint-switch.vercel.app` reached `Ready`, and `https://paintswitch.com` returned HTTP `200` and loaded the merged homepage. The homepage showed the chat prompt, and the widget opened with the title `PaintSwitch AI Assistant` and an AI-disclosing introduction. The launcher count was one on `/` and zero on `/privacy` and `/terms`, verifying the intended homepage-only placement in this check. A hidden file input existed but was not visible. Browser diagnostics recorded zero site/widget console logs after unrelated extension logs were excluded, including across the route checks.

The live Content Security Policy contained the exact HTTPS origins `https://widgets.leadconnectorhq.com`, `https://services.leadconnectorhq.com`, and `https://stcdn.leadconnectorhq.com`, plus exact `wss://services.leadconnectorhq.com` in `connect-src`; it retained `frame-src 'none'`. The expected security-header baseline was present and `X-Powered-By` was absent. No lead-delivery environment setting was changed, so the previously verified disabled state remains the latest evidence. No Production chat message was sent. This verifies Production deployment, homepage-only widget presence, opening, disclosure, and transport-policy availability only—not a Production reply or its latency. Ads and promotion remain off. Production response behavior, mobile/accessibility acceptance, exact delay/cap/sleep behavior, both kill switches, provider retention/anonymization, and any promoted release remain open.
...
## D-054 — Rename the customer-facing chatbot to Switch bot

- **Date:** 2026-08-08
- **Status:** Confirmed
- **Decision:** Name the GoHighLevel Conversation AI bot and its customer-visible widget `Switch bot`. The widget introduction and the first generated assistant reply must still explicitly identify Switch bot as PaintSwitch's AI assistant.
...
### D-058/D-059 hosted implementation evidence — 2026-08-10

This evidence records implementation verification only and creates no new owner decision. Commits `d3c1f6c` and `2ec63e1` on pull request #6 implement the approved editorial candidate, flexible public follow-up wording, and a cross-platform correction that serves the verified 1586×992 illustrative hero through its public `/images/` URL. All sixty-two tests, lint, TypeScript checking, and the Next.js `16.3.0` production build passed locally. At the 2026-08-10 checkpoint, GitHub `Verify`, Vercel, and Vercel Preview Comments all passed for application commit `2ec63e1`; the protected Vercel Preview reached Ready.
...
## D-068 — Expand organic SEO: topical content, additional Virginia/Maryland city pages, and backlink strategy; accept Maryland MHIC licensing risk

- **Date:** 2026-08-22
- **Status:** Confirmed
- **Decision:**
  **(A) Content/topical-authority plan.** Build a page cluster around the four already-approved services (interior, exterior, cabinet, commercial), split into commercial-intent service pages and non-salesy informational pages (e.g., color-selection or maintenance guidance). All new content follows the exact D-063 guardrails already governing city pages — no pricing, no license/EPA claims, no `Top-Rated` language, schema matching visible facts only, existing global design reused without modification.
  **(B) Additional city pages, Virginia and Maryland, in this priority order** (ranked by researched median household income within roughly 25 miles of the existing Alexandria/Arlington anchor pages): Chevy Chase Village MD, McLean VA, Potomac MD, Vienna VA, Fairfax Station VA, Bethesda MD, Great Falls VA, Oakton VA. Each new page follows the exact D-062/D-063 pattern: same four services, same rejected-claims list, same "page target is not a service boundary" framing; individual project review continues per PS-SEO-002. Build order and pacing (one or a few at a time, verified before continuing) to be set when implementation begins, matching the D-062 Alexandria-then-Arlington sequencing rather than publishing all eight at once.
  **(C) Backlink strategy.** Local citations/directories (Google Business Profile once created, Yelp, Angi, Nextdoor, Apple Maps, BBB) using PaintSwitch's service-area listing format, since no physical address is published (D-066). Real-estate-agent referral relationships as link/referral sources only, no co-branding or implied affiliation; candidates identified through research (not yet contacted): Patricia Ammann, Nick Gashel, Nadia Khan, The Davenport Group, and Ryan Mills, all active in the Alexandria/Arlington/McLean corridor. No paid link schemes or manipulative link tactics.
  **(D) Jen Contracting boundary.** No content, directory listing, citation, partnership page, backlink anchor text, or structured data produced under this decision may name, imply, or link to Jen Contracting, use "powered by Jen" language, share identity/registration details revealing the DBA relationship, or list Jen Contracting contact information in place of PaintSwitch's own. This applies to every third-party directory profile and partner-facing conversation, not just the website itself, consistent with D-001.
  **(E) Maryland MHIC licensing risk — knowingly accepted.** Research surfaced that Maryland's Home Improvement Commission (MHIC) licensing requirement appears to cover not just performing home-improvement work but advertising/soliciting it, that painting is explicitly covered "home improvement" work, and that out-of-state contractors are not exempt. This question was already flagged as open in D-036 ("jurisdiction-specific licensing... obligations remain subject to legal review") and was not part of the confirmed scope of the D-067 attorney review. Rather than resolving it through counsel before proceeding, the owner explicitly reviewed this finding and chose to accept the risk and move forward with Maryland city pages.
  - **Reason:** The owner selected the researched income-priority order and explicitly chose speed over further legal delay on the Maryland licensing question, mirroring the risk-acceptance approach already used in D-065 for the unfiled Virginia DBA exposure.
  - **Consequences:** This extends D-062/D-063's city-page pattern and D-062's "content priority, not a service boundary" framing to Chevy Chase, McLean, Potomac, Vienna, Fairfax Station, Bethesda, Great Falls, and Oakton. It authorizes a content/backlink workstream that previously had zero decision-log coverage. **It does not resolve the underlying MHIC legal question** — D-036's holding that Maryland licensing remains subject to legal review is still accurate and still open; this decision only declines to let that open question block the marketing work, and accepts the resulting regulatory exposure as a known, disclosed risk rather than a blocker. If Maryland actually accepts and performs a project (as opposed to only marketing), the MHIC question becomes materially more urgent and should go back to counsel before that work is contracted. No implementation (city pages, content, or outreach) has been built under this decision yet; each requires its own follow-through pass.
  - **Supersedes:** None identified — extends D-062/D-063's pattern to new cities and closes the zero-coverage content/backlink gap. Does not supersede D-036 (Maryland licensing remains open, not resolved) or D-001 (Jen Contracting boundary, restated above for this specific workstream).

## D-069 — Add Google Analytics (GA4) tracking to the website; disclose as a service provider

- **Date:** 2026-08-22
- **Status:** Confirmed
- **Decision:** Created a GA4 property ("PaintSwitch", Home & Garden industry, Small business size, Generate leads + Understand web/app traffic objectives) and a Web data stream for `https://paintswitch.com` (Measurement ID `G-LEKK2Q0XJ0`). Added the standard `gtag.js` snippet to `src/app/layout.tsx` via `next/script` with `afterInteractive` strategy, loaded on every route. No Google Signals, Ads linkage, or PII-bearing custom events were enabled — only default GA4 page-view/engagement measurement. Updated the Privacy Policy (`src/app/privacy/page.tsx`) to disclose Google Analytics as a service provider alongside the existing Vercel/GoHighLevel/Upstash/Microsoft 365 list, and bumped the Privacy Policy effective date to August 22, 2026 to reflect the new vendor.
  - **Reason:** Owner requested GA4 setup to measure website traffic and lead-generation performance now that real leads are flowing (post D-067 attorney approval, post D-068 city-page expansion). Adding a new data processor without updating the Privacy Policy's vendor disclosure would make that document inaccurate.
...
## D-073 — Remove the customer-facing chatbot from the website; cost-driven reversal of D-052/D-053/D-054/D-059

- **Date:** 2026-09-15
- **Status:** Confirmed
- **Decision:** The `Switch bot` GoHighLevel Conversation AI chat widget is removed from the live website. `HighLevelChatWidget` (`src/components/highlevel-chat-widget.tsx`) is deleted, its import and render call removed from the homepage, and the Content-Security-Policy conditional that added the LeadConnector widget/services/static origins only when the widget was enabled is removed entirely, since no page embeds it anymore. This does not touch lead delivery, which uses a separate server-side connection to the GoHighLevel REST API (`src/lib/ghl-client.ts`) unaffected by the chat widget's client-side script and CSP entries.
  - **Reason:** Owner decision on cost grounds. The chat feature runs on GoHighLevel's "AI Employee Unlimited" add-on, billed separately from the base "HighLevel Starter" subscription that the lead-generation pipeline (CRM, pipeline, workflows) depends on (see `DEVELOPMENT_STATUS.md`, 2026-08-06 entry: Starter $97/month, AI Employee Unlimited an additional $97/month after its trial). The owner judged the AI Employee cost not worth it and asked to remove the chatbot.
  - **Consequences:** This reverses D-052 (chatbot confirmed), D-053 (Live Chat/behavioral controls), D-054 (Switch bot identity), and D-059 (chatbot bundled into the full-design launch) — the customer-facing chatbot is no longer part of the live site. **Removing the website embed alone does not cancel the GoHighLevel subscription charge.** The owner still needs to cancel or downgrade the "AI Employee Unlimited" add-on inside GoHighLevel (Settings → Subscription/Billing) to stop the recurring charge; the base "HighLevel Starter" subscription must stay active, since it is what runs lead delivery, the CRM pipeline, and the owner-notification workflow. **Not resolved in this decision:** the Privacy Policy (`src/app/privacy/page.tsx`) still discloses the "Switch bot" chat by name (the live-chat data-collection paragraph and the AI-disclosure paragraph) — that page was part of the D-067 attorney review, so its wording was intentionally left unchanged pending explicit owner direction rather than edited silently alongside this code change. If the chatbot is not coming back, that disclosure should be revised or removed in a follow-up pass.
  - **Supersedes:** D-052, D-053, D-054, and the chatbot-inclusion portion of D-059. Does not affect D-067 (attorney-approved Privacy Policy and Terms) or lead-delivery decisions (D-018/D-023/D-024/D-025/D-030 etc.).
