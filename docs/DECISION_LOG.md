...
- **Consequences:** The three custom fields may be created in GoHighLevel and used by the website mapping. **Implementation verification — 2026-08-03:** The fields and local mappings exist, and the server-side field identifiers are configured. Staff ownership, notification destination/workflow, Upstash runtime, deployment, and provider end-to-end tests remain incomplete. The owner accepted the Microsoft Customer Agreement on 2026-08-02; mailbox creation, private credential entry, sender setup, and send/receive testing still must complete. The approved address must not be treated as working until those tests pass.
...
- **Consequences:** The Upstash account/database and server-only Vercel secrets may be configured and the lead-delivery path may use the approved submission ID for durable duplicate protection. Key design, retention/TTL, atomic operation, provider-failure behavior, reconciliation, limits, monitoring, and acceptance tests are implementation details to complete and verify within this confirmed boundary. No Upstash credential may enter customer-delivered code or tracked source. **Implementation verification — 2026-08-03:** Pushed commit `efb96b9` contains a token-checked 60-second lock, release of an acquired lock if the initial state read fails, durable delivery phases, streamed response-size caps, provider-failure reconciliation, explicit rejected-create recovery, and fault-injection tests for at-most-one automatic Opportunity creation and recovery from state-write failures. The Upstash database and secrets do not yet exist because the marketplace terms still require explicit owner acceptance. No real Upstash or GoHighLevel end-to-end delivery has passed.
