...
### Local real-time transport diagnosis — 2026-08-08

This is implementation evidence and an inference, not a new decision or hosted acceptance. Inspection of HighLevel's public widget bundle showed Socket.IO using marketplace `https://services.leadconnectorhq.com`, path `/sockets-live-chat/socket.io`, and WebSocket/polling transports. The uncommitted repository source now adds the exact `wss://services.leadconnectorhq.com` origin to chat-enabled `connect-src` without a wildcard, and the source-contract test now asserts that exact WSS origin. This is a plausible explanation for the delayed browser display observed on commit `649f06d`; it is not proven until a new hosted deployment reproduces or resolves the behavior.

After this uncommitted change, all 61 tests, lint, TypeScript checking, and the chat-enabled Next.js `16.3.0` production build passed. The build reported `/`, `/_not-found`, `/api/leads`, `/privacy`, and `/terms`. Commit `649f06d` and its existing Preview do not contain this WSS change. No hosted retest, new pull-request check, Production verification, or real-time-latency acceptance exists for the change yet.
