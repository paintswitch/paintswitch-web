...
- **Consequences:** The three custom fields may be created in GoHighLevel and used by the website mapping. **Implementation verification — 2026-08-03:** The fields and local mappings exist, and the server-side field identifiers are configured. D-029 confirms `hello@paintswitch.com` as the owner-notification destination, and a saved Draft notification workflow now exists, but staff ownership in GoHighLevel, workflow publication/testing, real Upstash runtime verification, production deployment, and provider end-to-end tests remain incomplete. Owner-supplied photographic evidence confirms that the separately sent test email with subject `New PaintSwitch website lead` reached the mailbox. This proves receipt of that test only, not complete inbound/outbound or workflow acceptance. The visible sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>` conflicts with D-001 and must be corrected to a verified PaintSwitch-only identity before publication or customer-facing use.
...
- **Consequences:** Configure the owner-notification workflow to use `hello@paintswitch.com`; never use `hell@paintswitch.com`. This decision confirms the destination only. **Implementation verification — 2026-08-03:** Saved Draft workflow `PaintSwitch Website Lead - Owner Notification` uses an Opportunity Created trigger filtered to `PaintSwitch Lead Intake` and an internal Email action directed to `hello@paintswitch.com`; it remains unpublished and its From defaults are blank. Owner-supplied photographic evidence confirms that a separately sent test email with subject `New PaintSwitch website lead` reached the PaintSwitch mailbox and displayed the approved internal-alert body. The screenshot displayed sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. The receipt proves that the mailbox could receive that separate test; it does not prove the Draft workflow, website-to-CRM delivery, or end-to-end notification path. The Jen-linked sender identity conflicts with D-001 and must be corrected and verified as PaintSwitch-only before workflow publication or customer-facing use. Lead delivery and notification must not be represented as operational until sender correction, workflow publication, failure handling, and end-to-end acceptance evidence are complete.
