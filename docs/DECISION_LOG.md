...
- **Consequences:** The internal owner-notification workflow must use exactly those From values and must not use a Jen Contracting-linked sender. This decision confirms only the internal notification sender choice. Branded-domain authentication, customer-facing sender scope, triggered-message inbox receipt, failure handling, and production end-to-end operation remain **TBD** or unverified as labeled. **Implementation verification — 2026-08-04:** The workflow contains From name `PaintSwitch` and From email `hello@paintswitch.com`. Owner-supplied screenshot evidence shows that the corrected controlled test reached `hello@paintswitch.com`, displayed visible sender name `PaintSwitch`, used routing address `hello+paintswitch.com@mail.msgsndrroute.com`, and matched the approved subject/body with no Jen identity. This satisfies AC-044's visible no-Jen sender evidence. A later controlled manual Opportunity and one protected Preview website submission each verified one workflow trigger and successful internal-notification action, but neither independently verified inbox receipt for its triggered message, branded-domain authentication, or failure recovery. D-032 separately authorized and verified publication.
