...
- **LB-026 — Confirmed:** Send owner lead notifications to `hello@paintswitch.com`. The 2026-08-03 owner clarification resolves the temporary `hell@paintswitch.com` typo conflict. Owner-supplied photographic evidence confirms receipt of an earlier separately sent test email at the mailbox, but that test displayed sender `Paint Switch <alex+jencontracting.com@mail.msgsndrroute.com>`. `LB-029` now confirms the replacement internal sender values, which are saved in the Draft. A controlled second test send completed in the GoHighLevel UI without a visible error, but its receipt and actual inbox sender line remain unverified. Notification is not operational until sender authentication/deliverability, workflow publication/testing, failure handling, and end-to-end acceptance pass.
...
- **LB-029 — Confirmed:** Use From name `PaintSwitch` and From email `hello@paintswitch.com` for the internal owner lead-notification email. Approved on 2026-08-03 under D-031. Authentication, deliverability, the actual inbox sender line, customer-facing sender scope, workflow publication, failure handling, and end-to-end operation remain **TBD** or unverified as labeled.
...
- **PL-011 — TBD:** Approve or supply notification-workflow publication/testing and failure handling, payment-processor configuration, post-beta enabled modules, operating ownership not already assigned, sender authentication/deliverability, customer-facing sender identity, A2P legal-business/DBA and authorized-representative facts, commercially compliant Vercel plan, and release phasing. `LB-026` confirms the notification destination, and `LB-029` confirms the internal notification From name and email. A saved Draft workflow contains those From settings but is unpublished and untested as a workflow. A controlled second test send completed in the GoHighLevel UI without a visible error, but receipt and the actual inbox sender line remain unverified. The private integration, server-only GHL and Upstash configuration, field creation, APIs, mappings, and D-030 safeguards are implemented and locally verified under `LB-020` and `PL-018` through `PL-020`; hosted safeguard verification, notification operation, production promotion, and provider acceptance remain incomplete.
...
- **PL-017 — Confirmed:** Use `hello@paintswitch.com` as the primary PaintSwitch domain mailbox. Owner-supplied photographic evidence confirms receipt of an earlier separate test email, proving that mailbox could receive that test. The Draft now has the D-031 internal sender values saved, and a controlled second test send completed in the GoHighLevel UI without a visible error. Receipt of the second test, its actual inbox sender line, complete outbound behavior, authentication/deliverability, and workflow acceptance remain unverified; do not treat the Draft workflow as operational on this evidence.
...
9. **TBD:** Notification-workflow publication/testing and failure handling, verification of the configured PaintSwitch-only sender's authentication, deliverability, and actual inbox sender line, backup owner, measurable human-response target and operating hours, exact SMS and consent wording/workflow details, final legal approval, broader CRM/business/customer-data retention and deletion, and provider end-to-end verification. The destination and internal From values are confirmed; receipt of an earlier separate test email is confirmed; a controlled second test send completed in the GoHighLevel UI without a visible error but has no owner-confirmed receipt; the Draft workflow remains unpublished; and the private integration, GHL identifiers/fields, Upstash database/variables, mapping code, 30-day technical Upstash-record TTL, and five-valid-submissions-per-client-IP-per-600-seconds rate limit are implemented and locally verified. Hosted verification remains pending.
