...
- **LB-026 — Confirmed:** Send owner lead notifications to `hello@paintswitch.com`. The 2026-08-03 owner clarification resolves the temporary `hell@paintswitch.com` typo conflict. The destination is approved and a draft workflow is saved, but notification is not operational until the mailbox is provisioned and tested, the workflow is published and tested, failure handling is verified, and end-to-end acceptance passes.
...
The confirmed GoHighLevel selection, CRM foundation, and server-side connection boundary do not by themselves prove production operation. As of 2026-08-03, the private integration, required permissions, Sensitive Vercel GHL configuration, location/pipeline/stage identifiers, seven field identifiers, four additional Opportunity fields, API/mapping code, free Upstash database, five Sensitive Upstash variables, and saved draft `PaintSwitch Website Lead - Owner Notification` workflow are verified. The draft filters Opportunity Created to `PaintSwitch Lead Intake` and prepares an internal Email to `hello@paintswitch.com`, but it is unpublished and untested. The candidate is available in a fail-closed Vercel Preview. `LEAD_DELIVERY_ENABLED` remains `false`; retention/TTL is unresolved; and no provider end-to-end delivery, owner notification, payment processor, or production messaging path is verified. GoHighLevel's intended AI-chat function also does not resolve the customer-facing chatbot's model, architecture, exact interface, or phasing.
...
9. **TBD:** Notification-workflow publication/testing and failure handling, mailbox activation/testing, backup owner, measurable human-response target and operating hours, exact SMS and consent wording/workflow details, final legal approval, retention/TTL, and provider end-to-end verification. The destination is confirmed as `hello@paintswitch.com`; a draft notification workflow exists; and the private integration, GHL identifiers/fields, Upstash database/variables, and mapping code are verified.
...
- **Confirmed:** GoHighLevel and its intended CRM function are approved. The private-integration method, server-side Vercel token location, free Upstash Redis idempotency provider, four D-026 Opportunity field names/objects, and `hello@paintswitch.com` owner-notification destination are also approved. Beta permissions, GHL and Upstash variable names, field creation, APIs, mappings, and an unpublished notification-workflow draft are configured; the code is available in a fail-closed Vercel Preview, `LEAD_DELIVERY_ENABLED` is `false`, and retention/TTL remains unresolved. Payment-processor setup, post-beta modules, notification publication/testing, sender identity, and phasing remain **TBD**.
