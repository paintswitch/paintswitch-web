...
- **LB-026 — Confirmed:** Send owner lead notifications to `hello@paintswitch.com`. The 2026-08-03 owner clarification resolves the temporary `hell@paintswitch.com` typo conflict. The destination is approved, the owner reports the mailbox working, and a test email was sent, but receipt remains unconfirmed and the saved workflow remains Draft and unpublished. Notification is not operational until receipt, workflow publication/testing, failure handling, and end-to-end acceptance pass.
...
- **PL-017 — Confirmed:** Use `hello@paintswitch.com` as the primary PaintSwitch domain mailbox. The owner reports it working and a test email was sent, but receipt and complete inbound/outbound acceptance remain unconfirmed; do not treat the workflow as operational on that evidence alone.
...
9. **TBD:** Notification-workflow publication/testing and failure handling, confirmation that the test email was received, backup owner, measurable human-response target and operating hours, exact SMS and consent wording/workflow details, final legal approval, broader CRM/business/customer-data retention and deletion, and provider end-to-end verification. The destination is confirmed as `hello@paintswitch.com`; a Draft notification workflow exists; the owner reports the mailbox working; and the private integration, GHL identifiers/fields, Upstash database/variables, mapping code, 30-day technical Upstash-record TTL, and five-valid-submissions-per-client-IP-per-600-seconds rate limit are implemented and locally verified. Hosted verification remains pending.
