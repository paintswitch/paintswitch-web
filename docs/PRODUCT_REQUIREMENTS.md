...
- **LB-041 — Confirmed:** Keep `hello@paintswitch.com` as the immediate primary internal destination for every accepted beta lead. Alex must not receive every lead and may receive only backup/escalation internal alerts after `LB-040` activation conditions pass. Automated customer SMS remains off. Exact escalation triggers, timing, GoHighLevel routing configuration, and the escalation acknowledgment mechanism remain **TBD** or unverified.
- **LB-042 — Proposed:** Add a second internal `PaintSwitch Lead Intake` stage named `Contact Attempted` as one possible aid for backup/escalation routing. This stage is not approved or implemented. `PL-013` remains controlling with exactly one `New Quote Request` stage until explicit owner approval.

The beta CRM foundation is governed by `PL-012` through `PL-016`. Protected Preview evidence verifies secure server-side delivery, one owner-confirmed inbox receipt, one fail-closed/retained-submission recovery completed exactly once, and the hosted rate threshold. The current candidate implements the customer-facing portions of `LB-032` through `LB-038` through dated Privacy and Terms routes and a pre-submit disclosure. `LB-041` is approved policy only; its exact routing and acknowledgment behavior remain unimplemented. `LB-042` is Proposed and must not be implemented without approval. Professional/legal review, `LB-034`/`LB-035` operating rehearsal, `LB-006` Production acceptance, hosted concurrency, notification-action failure recovery, and conditional-backup operation remain incomplete.
...
- **Proposed implementation note:** A second `Contact Attempted` stage is proposed under `LB-042`; it is not part of the Confirmed pipeline and must not be added without explicit owner approval.
...
- **AC-052 — Confirmed:** A production-safe routing test proves every accepted beta lead immediately notifies `hello@paintswitch.com`, routine leads do not notify Alex, and only a separately approved backup/escalation condition can produce an internal Alex alert after `LB-040` activation. The test sends no automated customer SMS. This criterion cannot pass until the escalation triggers, routing configuration, and acknowledgment mechanism are approved and verified.
...
Decisions D-018 through D-048 confirm the beta as the first release and define its ownership, contact and SMS boundaries, hosting and Pro plan, public domain, Virginia operating focus, internal/legal DBA direction with strict public separation, CRM foundation, private-integration/token boundary, Upstash idempotency, additional Opportunity fields, owner-notification destination, internal sender, workflow publication, technical-state TTL, rate-limit threshold, privacy and retention policy, age restriction, governing-law terms, response target, conditional backup, backup/escalation-only Alex routing, target date, owner-time limit, custom-form/server-side integration boundary, review-content rule, and dependency remediation. The remaining end-state outcomes still govern later releases but do not block this beta unless listed in the approved beta requirements. The 2026-08-08 date is a target and never overrides an applicable launch gate.
