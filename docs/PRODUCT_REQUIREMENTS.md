...
- **LB-029 — Confirmed:** Use From name `PaintSwitch` and From email `hello@paintswitch.com` for the internal owner lead-notification email. Approved on 2026-08-03 under D-031. The corrected received test displayed visible sender name `PaintSwitch`, routing address `hello+paintswitch.com@mail.msgsndrroute.com`, and no Jen identity, satisfying AC-044. One later controlled manual Opportunity verified the configured trigger and one successful internal-notification action. Branded-domain authentication, customer-facing sender scope, triggered-message inbox receipt, failure handling, and end-to-end operation remain **TBD** or unverified as labeled.
- **LB-030 — Confirmed:** Publish the exact `PaintSwitch Website Lead - Owner Notification` workflow after verifying its approved trigger filter, recipient, From values, subject, and body. Approved and externally verified on 2026-08-04 under D-032. A controlled manual Opportunity subsequently verified exactly one enrollment, one successful internal-notification action, and workflow completion. This does not prove triggered-message inbox receipt, failure recovery, staff ownership, authentication, website delivery, or website-to-CRM-to-notification operation.
...
- **PL-011 — TBD:** Approve or supply remaining notification failure handling, payment-processor configuration, post-beta enabled modules, operating ownership not already assigned, branded-domain sender authentication, customer-facing sender identity, A2P legal-business/DBA and authorized-representative facts, commercially compliant Vercel plan, and release phasing. `LB-026`, `LB-029`, and `LB-030` confirm the notification destination, internal sender, and publication of the exact workflow. The workflow remained Saved and published after reload, the corrected controlled email was received without a visible Jen identity, and a later controlled manual Opportunity produced one completed enrollment and successful internal-notification action. Triggered-message inbox receipt and website-to-CRM notification remain unverified. The private integration, server-only GHL and Upstash configuration, field creation, APIs, mappings, and D-030 safeguards are implemented and locally verified under `LB-020` and `PL-018` through `PL-020`; hosted safeguard verification, complete notification acceptance, production promotion, and provider acceptance remain incomplete.
...
**AC-041 implementation evidence — 2026-08-04:** An isolated synthetic Contact and manual Opportunity triggered exactly one workflow enrollment; the internal-notification action reported `Success`, and the run finished. This partially satisfies AC-041 by verifying the configured trigger and one successful action. AC-041 remains open because inbox receipt for that triggered message was not independently observed, the failure-recovery path was not exercised, and the website-to-CRM path remains disabled and untested.
