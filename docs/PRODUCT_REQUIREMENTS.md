...
- **PS-SEO-001 — Confirmed:** Build local SEO city pages for Alexandria, Virginia first and Arlington, Virginia second. Approved on 2026-08-14 under D-062.
- **PS-SEO-002 — Confirmed:** Treat each city page as a marketing/content target rather than proof of an exact service boundary. Retain individual service-area and project review; exact eligible cities, counties, ZIP codes, travel zones, and permanent boundaries remain **TBD**.
- **PS-SEO-003 — Rejected:** Do not request or publish a state-license number or EPA Lead-Safe claim for PaintSwitch city landing pages. The owner rejected those template inputs on 2026-08-14 and directed that they not be requested again.
- **PS-SEO-004 — Confirmed:** Preserve all existing global design and shared component structure for city pages. Use page-local content only; show only interior, exterior, cabinet, and commercial painting; keep metadata and Service/FAQ structured data aligned with visible facts; and exclude unsupported numeric prices, `Top-Rated` language, guaranteed coverage, or invented claims.
- **PS-SEO-005 — Confirmed:** Publish the approved Alexandria and Arlington city pages and sitemap entries to Production. Approved and completed/verified on 2026-08-14 (America/New_York) under D-064. Publication does not establish an exact service boundary or prove form delivery, indexing, visual/responsive/accessibility quality, console cleanliness, performance, or promoted-traffic readiness.
...
- **FR-036 — Confirmed:** Render static Alexandria and Arlington local SEO pages in that order of priority, using the existing shared visual system and quote-request path without changing global design components.
- **FR-037 — Confirmed:** Keep city-page visible copy, metadata, and JSON-LD synchronized around the four approved services and individual review. Do not emit a state-license number, EPA Lead-Safe claim, unsupported numeric price, `Top-Rated` claim, guaranteed ZIP/address coverage, rating, or other unverified trust field.
...
- **AC-062 — Confirmed:** Static `/alexandria-va` and `/arlington-va` pages use title tags under 60 characters, meta descriptions under 155 characters, canonical URLs, visible FAQs that match FAQ structured data, and Service structured data limited to the same four services shown on the page.
- **AC-063 — Confirmed:** City pages reuse the established global components and styling without editing global design files; local navigation anchors remain present; no unsupported numeric price, `Top-Rated`, state-license, EPA Lead-Safe, Jen relationship, or guaranteed-coverage claim appears in visible content or structured data.
...
- **Confirmed:** Alexandria and Arlington city-page targets do not resolve exact service-area eligibility; neighborhood and ZIP references are context only and every address remains subject to individual review.
- **Rejected:** State-license and EPA Lead-Safe template claims or requests are outside PaintSwitch city-page scope and must not be added or requested.
